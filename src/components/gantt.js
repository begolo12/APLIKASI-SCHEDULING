...
export function renderGantt(container, cards, onEventClick, onUpdateCard) {
  ...
  orderedCards.forEach(({ card, isSub, no }) => {
    ...
    if (zoomMode === 'day') {
      ...
      if (itemStartMs > monthEndMs) return;
      if (itemDueMs < monthStartMs) return;
      ...
      // Month mode: Check if item overlaps viewYear
      if (startVal.getFullYear() > viewYear || dueVal.getFullYear() < viewYear) return;
